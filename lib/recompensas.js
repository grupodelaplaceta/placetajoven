...
async function listar() {
  const c = conf();
  const recompensas = c.on
    ? await listarSupabase(c)
    : DEMO.map(demoPublica);
...
async function listarSupabase(c) {
  ...
  try {
    ...
    return DEMO.map(demoPublica);
  }

  if (!res.ok) {
    // 404 = tabla aún no creada → catálogo de ejemplo para no romper la UI.
    if (res.status === 404) return DEMO.map(demoPublica);
